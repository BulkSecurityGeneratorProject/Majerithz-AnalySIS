...
export class CompanyService {
    ...
    private convertDateFromClient(company: ICompany): ICompany {
        const copy: ICompany = Object.assign({}, company, {
            creationDate: company.creationDate != null && company.creationDate.isValid() ? company.creationDate.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((company: ICompany) => {
            company.creationDate = company.creationDate != null ? moment(company.creationDate) : null;
        });
        return res;
    }
}
